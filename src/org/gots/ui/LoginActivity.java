	...
	protected void onResume() {
		...
		buttonLayout.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(LoginActivity.this, getResources().getString(R.string.feature_unavalaible), Toast.LENGTH_SHORT).show();
...
//				finish();
		...
		connect.setOnClickListener(new View.OnClickListener() {
			...
			protected void basicNuxeoConnect() {
				...
				if (token == null) {
					...
					GotsPreferences.getInstance(LoginActivity.this)
							.setNUXEO_PASSWORD(passwordText.getText().toString());
		...
		AsyncTask<Object, Void, String> task = new AsyncTask<Object, Void, String>() {
			...
			protected String doInBackground(Object... objects) {
				try {
					...
					client.setRequestInterceptor(new TokenRequestInterceptor("myApp", "myToken", "myLogin", GotsPreferences.getInstance(LoginActivity.this).getDeviceId()));

					Session session = client.getSession();

					Documents docs = (Documents) session.newRequest("Document.Email")
							.setHeader(Constants.HEADER_NX_SCHEMAS, "*").set("email", email).execute();

//					String uri = GotsPreferences.getInstance(getApplicationContext())
//							.getGardeningManagerNuxeoAuthentication();
//
//					List<NameValuePair> params = new LinkedList<NameValuePair>();
//					params.add(new BasicNameValuePair("deviceId", GotsPreferences.getInstance(getApplicationContext())
//							.getDeviceId()));
...
//					urlConnection.addRequestProperty("X-Device-Id", GotsPreferences
...
//							"Authorization",
//							"Basic "
//									+ Base64.encodeToString((loginText.getText().toString() + ":" + passwordText
