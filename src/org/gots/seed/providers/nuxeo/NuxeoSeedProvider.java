			...
			AsyncTask<BaseSeedInterface, Integer, Document> task = new AsyncTask<BaseSeedInterface, Integer, Document>() {
				...
				protected Document doInBackground(BaseSeedInterface... params) {
                    ...
                    props.set("vendorseed:datesowingmax",
                            String.valueOf(currentSeed.getDateSowingMax()));
                    props.set("vendorseed:durationmin",
                    ...
                    try {
                        ...
                        try {
                            Document root;
                            root = (Document) session.newRequest(
                                    "Document.Fetch").set("value", wsRef).execute();
                            folder = (Document) session.newRequest(
                                    "Document.Create").setInput(root).setHeader(
                                    Constants.HEADER_NX_SCHEMAS, "*").set(
                                    "type", "Hut").set("name", "Catalog").set(
                                    "properties", "dc:title=" + "Catalog").execute();
                            catalog = folder;

                            Log.d(TAG,
                                    "doInBackground create folder catalog UUID "
                                            + folder.getId());
                        } catch (Exception e1) {
                            Log.e(TAG,
                                    "Creating folder Catalog" + e.getMessage());
                        }
...
>>>>>>> temp
                    try {
                        ...
                        documentVendorSeed = (Document) session.newRequest(
                                ...
                                "VendorSeed").set("name",
                                currentSeed.getVariety()).set("properties",
                                props).execute();

                        Log.d(TAG, "doInBackground remoteSeed UUID "
                                + documentVendorSeed.getId());
                    } catch (Exception e) {
                        // TODO Auto-generated catch block
                        e.printStackTrace();
					}
